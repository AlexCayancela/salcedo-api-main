...
export default function PantallaListaReserva({route,navigation}) {
    ...
    const [idTurismo, setidTurismo] = useState('');
    ...
    const cambiarEstado = async (idReserva) => {
     ...
     function actualizarCupos(idTura,cuposa){
      //  alert(idTura)
       setcupos(cuposa??0);
       setidTurismo(idTura);
       setIsVisible(true);
     ...
     const updateCupos = async () => {
      try {
       const response = await fetch(UrlHost+'actualizar-cupo-turismo/'+idTurismo+"/"+cupos,{
        headers: {
          'Accept': 'application/json, text/plain, */*',  // It can be used to overcome cors errors
          'Content-Type': 'application/json'
        }
                            ...
                            <DataTable>
                              <DataTable.Row>
                               ...
                               {'  '}
                                ...
                                onPress={() => actualizarCupos(l.idTurismo,l.cupos)}
                              />
